...
export class PostgresUserRepository implements UserRepositoryPort {
   ...
   *
   * Creates a new user record in PostgreSQL with all entity properties. If the user
   * entity does not have an ID, the database will generate a UUIDv7 using the uuidv7()
   * function (PostgreSQL 18+). The method returns the generated or provided ID.
   *
   * @async
   * @param {User} userEntity - The user domain entity to persist
   * @returns {Promise<string>} The user ID (generated by database if not provided)
   *
   ...
   * const user = new User(undefined, email, password, 'John Doe', role, new Date())
   *
   * try {
  ...
  async update(userEntity: User): Promise<void> {
    if (!userEntity.id) {
      throw new DatabaseException('Cannot update user without ID', {})
    }
